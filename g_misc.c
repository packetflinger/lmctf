...
*/
void SP_func_explosive (edict_t *self); //predeclare surt
...
void func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	//surt, why was this commented out?????
	// bmodel origins are (0 0 0), we need to adjust that here
	VectorScale (self->size, 0.5, size);
	VectorAdd (self->absmin, size, origin);
//	VectorCopy (origin, self->s.origin);
	//surt, don't copy into the ent's origin!  the ent needs to reappear
	//this information is only calculated for the throwdebris!
	

//	VectorCopy(self->s.origin, origin); //surt FIXME put something in origin
	...
	//surt this is commented out because we don't want to screw with
	//self while generating the debris
...
//	VectorScale (self->velocity, 150, self->velocity);
	...
	// LM_JORM -- Support respawning explosive walls
	if (!deathmatch->value)
	{
		if (self->dmg)
...
void SP_func_explosive (edict_t *self)
{
	...
	*/
	...
	{
//		if (!self->health)
		//surt health might be less than 0 on respawn
		if (self->health <= 0)
			self->health = 100;
...
// LM_JORM -- COMPATIBILITY WITH ZOID'S BANNERS
...
// END LM_JORM -- COMPATIBILITY WITH ZOID'S BANNERS
